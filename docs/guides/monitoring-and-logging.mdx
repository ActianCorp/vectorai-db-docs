...
The following sections describe every metric exposed by the `/metrics` endpoint, grouped by category. All metrics use the prefix **`actian_vectorai_`**. The full metric name is `actian_vectorai_<name>`. For example, `actian_vectorai_collections_total`, `actian_vectorai_rest_responses_total`.
...
|--------|------|-------------|
| `memory_active_bytes` | Gauge | jemalloc active memory (or working set on Windows) |
| `memory_allocated_bytes` | Gauge | jemalloc allocated memory (or private bytes on Windows) |
| `memory_metadata_bytes` | Gauge | jemalloc metadata overhead (Linux only) |
| `memory_resident_bytes` | Gauge | Resident set size |
| `memory_retained_bytes` | Gauge | jemalloc retained unmapped memory (Linux only) |
...
- The `memory_active_bytes`, `memory_allocated_bytes`, `memory_metadata_bytes`, and `memory_retained_bytes` metrics are only registered/emitted when jemalloc is enabled (`VDE_HAVE_JEMALLOC`). They are omitted entirely from builds that don't link jemalloc.

- The metric `process_memory_free_bytes` is sourced directly from the OS (Windows `GlobalMemoryStatusEx`, Linux `sysinfo`). It reflects machine-wide available RAM, independent of the process’s own usage metrics.
...
The following Prometheus Query Language example queries demonstrate common monitoring patterns that you can use in Grafana or any Prometheus-compatible dashboard tool.
