...
Install the Actian VectorAI DB client:

```bash
pip install actian-vectorai-client
```

### Dependency conflict with scalekit-sdk-python

`scalekit-sdk-python==2.12.0` pins `protobuf<7.0.0`. `actian-vectorai-client` requires `protobuf>=6.31.1`. When pip resolves these together, it downgrades protobuf to a version that causes a runtime import error:
...
This is not Python-version-specific. To work around it, install the packages in the following order:

```bash
# Install everything except scalekit first
grep -v scalekit-sdk-python requirements.txt > /tmp/req.txt
pip install -r /tmp/req.txt

# Install scalekit without resolving its declared dependencies
pip install scalekit-sdk-python==2.12.0 --no-deps

# Reinstate the versions actian-vectorai-client requires
pip install "protobuf>=6.31.1" "grpcio-status>=1.67.0"
```

<Warning>
`scalekit-sdk-python` is pinned to `2.12.0`. Before upgrading to a newer version, manually verify the new version's transitive dependencies against your `requirements.txt`.
</Warning>
...
`scalekit-sdk-python==2.12.0` and `actian-vectorai-client` pin incompatible protobuf ranges. Follow the installation order in [Installation](#installation) to avoid a runtime `VersionError`. Upgrading either package requires re-checking transitive dependencies.

**`scalekit-sdk-python` version pin**  
This integration is verified against `scalekit-sdk-python==2.12.0`. Before upgrading, check whether the new version's dependencies conflict with `actian-vectorai-client`.
