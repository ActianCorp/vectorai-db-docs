---
title: "Quickstart"
description: "Create a collection, insert vectors, and run your first similarity search."
---

import Prerequisites from "/snippets/python-prerequisites.mdx";

Before starting, make sure VectorAI DB is running and your SDK is installed. See the [Installation guide](/home/installation/instructions) if you haven't done that yet.
...
Connect to VectorAI DB and create a collection named `products` with dimension 128 and cosine distance metric. Save each step below to a single file called `quickstart.ts` and run it with `npx tsx quickstart.ts`.
...
<CardGroup cols={2}>
  ...
  <Card title="Python SDK reference" icon="code" href="/sdks/python/reference">
    Namespaces, configuration, filters, and error handling.
  </Card>
  <Card title="JavaScript SDK reference" icon="code" href="/sdks/javascript/reference">
