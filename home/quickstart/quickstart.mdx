...
```
Port 6573 is for the RESTful API, 6574 for gRPC, and 6575 for LocalUI. The /var/lib/actian-vectorai directory is the containers location for storing Actian VectorAI DB. It should be volume-mounted to persist the data.

This command hosts the RESTful API on **localhost:6573**, the server on **localhost:6574**, and the LocalUI on **localhost:6575**.
...
from actian_vectorai_client import PointStruct
