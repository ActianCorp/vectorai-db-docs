...
Actian VectorAI provides VDE (Vector Data Engine) operations for managing collections. Use `get_vector_count` to check collection size and `flush` to persist data to disk (already shown in step 5). To remove a collection entirely, call `client.collections.delete(COLLECTION)`.
...
<CardGroup cols={2}>
 ...
 Learn how to build filters with `must`, `should`, and `must_not`
 </Card>
