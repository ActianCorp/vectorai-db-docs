...
- Actian VectorAI DB is running and accessible at `localhost:6574` (see [installation guide](/docs/installation/index)).
...
```bash
pip install deepface opencv-python actian-vectorai-client
```
...
import numpy as np
import cv2
from actian_vectorai_client import AsyncVectorAIClient, VectorParams, Distance, PointStruct
from deepface import DeepFace

async def create_face_collection():
    async with AsyncVectorAIClient("localhost:6574") as db:
...
<Note>
Verify with your Actian engineering team that `AsyncVectorAIClient`, `VectorParams`, `Distance`, and `PointStruct` are exported from `actian_vectorai_client` with the method signatures used here before running in production.
</Note>
...
```python
from actian_vectorai_client import Filter, Conditions
