...
Stage 3: Return the top-K from the rerankscored list
```
...
from actian_vectorai_client.models.collections import (
    HnswConfigDiff,
    ScalarQuantization,
    QuantizationConfig,
)
from actian_vectorai_client.models.enums import Fusion
...
# Bi-encoder for first-stage retrieval, cross-encoder for precision reranking
...
=== Baseline (single-pass) ===
  1. id=7   score=0.7234  Transformer models use multihead self-attention to process sequences i...
...
async def server_rerank(query: str, prefetch_limit: int = 50, top_k: int = 5):
    ...
    async with AsyncVectorAIClient(url=SERVER) as client:
        # Prefetch retrieves a broad candidate pool; the outer query rerankscores them
...
Final query stage:
  Rescore the 50 candidates with the same vector  →  top 5
```
...
| `rescore=True` | Recompute scores from original float32 vectors—accurate |
...
=== Quantized (no rescore) ===
  1. id=7   score=0.7198  Transformer models use multihead self-attention to pro...
...
=== Quantized + Rescore (2x oversampling) ===
  1. id=7   score=0.7234  Transformer models use multihead self-attention to pro...
...
------------------------------------------------------------------------------------------
   1    8.9234    0.7234  Transformer models use multihead self-attention to ...
...
----------------------------------------------------------------------------------------------------
   1      0.8456    0.7234    97   2025  Transformer models use multihead self-attenti...
...
async def cascaded_rerank(query: str, top_k: int = 5):
    ...
    async with AsyncVectorAIClient(url=SERVER) as client:
        ...
        # Stage 2 (outer prefetch): tighter rerankscore with higher ef
...
print("=== Cascaded 3-Stage Reranking ===")
...
Stage 2 (outer prefetch):
  hnsw_ef=256  →  rerankscore 25, keep 15 (more accurate)

Stage 3 (final query):
  exact=True  →  rerankscore 15, keep 5 (exact, most accurate)
```
...
async def rerank_with_threshold(query: str, threshold: float = 0.5, top_k: int = 10):
    ...
    async with AsyncVectorAIClient(url=SERVER) as client:
        # score_threshold on the outer query prunes results after reranking
        results = await client.points.query(
            COLLECTION,
            query=vec,
            prefetch=[
...
| `score_threshold` on a `PrefetchQuery` | Applied within that prefetch stage, use to prefilter noisy candidates |
...
async def full_rerank_pipeline(
    ...
    # Stage 2: Cross-encoder rerankscoring
    pairs = [(query, c.payload.get("text", "")) for c in candidates]
    ce_scores = cross_encoder.predict(pairs)
...
Stage 2 — Rescore (server-side):
  Vector rerankscore via query() → top 10
...
---------------------------------------------------------------------------------------------------------
   1      0.9123    0.7234    8.9234    97  Transformer models use multihead self-att...
