...
- VectorAI DB running and accessible. To start a local instance:
  ...
  ```bash
  docker run -p 6574:6574 actian/vectorai-db
  ```

  The SDK connects over gRPC on port `6574` by default. If your instance uses a different host or port, replace `localhost:6574` in the examples below.
...
```python
import asyncio
from actian_vectorai_client import AsyncVectorAIClient, PointStruct
import random
 ...
 **Solution**: Start the server with `docker run -p 6573-6575:6573-6575 actian/vectorai:latest`
 </Accordion>
