---
title: "Building a multi-model system"
description: "Work with multiple embedding models in one system"
---

This tutorial builds a multi-model search system that stores text and image embeddings in a single Actian VectorAI DB collection, searches each vector space independently, and fuses the results using server-side Reciprocal Rank Fusion (RRF) and Distribution-Based Score Fusion (DBSF). The example uses a product catalog with two embedding models: `all-MiniLM-L6-v2` for text (384-dim) and `clip-ViT-B-32` for images (512-dim).
...
This is the core multi-model search pattern. The function below prefetches 20 candidates from each vector space, then passes both lists to the server for RRF fusion, returning a single merged ranking.

```python
async def multimodal_search_rrf(query: str, top_k: int = 5):
    """Multi-model search: prefetch from text and image spaces, fuse with RRF."""
...
print(f"Query: {query}")
print(f"\n=== Multi-model results (RRF fusion) ===")
for r in results:
...
The function below combines multi-model RRF fusion with structured payload filters. It builds a filter from optional `category` and `max_price` arguments and passes it to the outer `query()` call so it applies after the two prefetch stages have been fused.
...
print("=== Filtered multi-model search ===")
...
async def client_side_fusion_search(query: str, alpha: float = 0.7, top_k: int = 5):
    """
    Multi-model search with client-side fusion.
...
Each individual search requires a network round-trip. `search_batch` sends all queries in a single gRPC call, reducing total latency—especially important when searching multiple vector spaces for comparison or multi-query interfaces.
...
When simple fusion is not enough, you can nest prefetch stages to build a multi-stage retrieval pipeline. The function below retrieves candidates from each vector space, fuses them with RRF, and then re-ranks the merged list by text similarity to give the semantic model the final say.
...
async def nested_prefetch_search(query: str, top_k: int = 5):
    ...
    async with AsyncVectorAIClient(url=SERVER) as client:
        results = await client.points.query(
            COLLECTION,
            query=text_vec,   # Stage 3: re-rank by text similarity
            using="text",
            prefetch=[
                PrefetchQuery(
...
print("=== Nested prefetch: fuse then re-rank ===")
for r in results:
    print(f"  id={r.id}  score={r.score:.4f}  {r.payload.get('name')}")
```

The diagram below traces the execution flow from raw candidate retrieval through fusion to final re-ranking.
...
Stage 3 (final query):
  └─ Re-rank 15 candidates by text similarity → top 5
```
...
```text
=== Nested prefetch: fuse then re-rank ===
...
print("=== Tuned multi-model search (DBSF, per-space hnsw_ef) ===")
for r in results:
    print(f"  id={r.id}  score={r.score:.4f}  {r.payload.get('name')}")
```
...
The following patterns recap the core multi-model techniques covered in this tutorial. Use them as a quick reference when building your own pipelines.
...
Nest an RRF-fusion prefetch inside the outer query and set `query=text_vec` to fuse first, then re-rank by the text vector.
