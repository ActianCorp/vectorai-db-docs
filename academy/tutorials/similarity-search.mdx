...
- Fetch and count points using `points.get` and `points.scroll`.
...
When a collection uses scalar or product quantization, `QuantizationSearchParams` controls how quantized vectors are used during the search.
...
async def paginated_search(query: str):
    """Fetch results in pages using client-side slicing."""
    vec = embed_text(query)
    # Fetch enough results to fill multiple pages
    async with AsyncVectorAIClient(url=SERVER) as client:
        all_results = await client.points.search(
            COLLECTION, vector=vec, limit=PAGE_SIZE * 3, with_payload=True,
        ) or []
...
asyncio.run(paginated_search("programming languages and frameworks"))
```
...
Results are always ranked by similarity score before the window is applied. Slice the result list on the client side to retrieve different pages.
...
The following snippet counts the total number of points in the collection, then counts filtered subsets by topic and difficulty.

```python
async def count_examples():
    async with AsyncVectorAIClient(url=SERVER) as client:
        # Total count via vde.get_vector_count
        total = await client.vde.get_vector_count(COLLECTION)
        print(f"Total points: {total}")

        # Filtered counts — scroll all points and count in Python
        all_pts, _ = await client.points.scroll(
            COLLECTION, limit=1000, with_payload=True, with_vectors=False,
        )

    dl_count  = sum(1 for p in all_pts if p.payload.get("topic") == "deep_learning")
    beginner  = sum(1 for p in all_pts if p.payload.get("difficulty") == "beginner")
...
```text
Total: 10  deep_learning: 2  beginner: 3
```

## Step 11: Running multiple searches
...
async def multi_search():
    ...
    async with AsyncVectorAIClient(url=SERVER) as client:
        for q in queries:
            vec = embed_text(q)
            results = await client.points.search(
                COLLECTION, vector=vec, limit=3, with_payload=True,
            ) or []
            print(f"\nQuery: {q}")
            for r in results:
                print(f"  id={r.id}  score={r.score:.4f}  {r.payload.get('text')[:60]}...")
...
for r in results:
    vec = r.vectors if isinstance(r.vectors, list) else []
...
```text
=== Search with vectors ===
  id=1  score=0.5571  dim=384  topic=machine_learning
    first 5 values: [-0.0868, -0.0287, 0.0288, 0.055, 0.0628]
  id=2  score=0.3018  dim=384  topic=deep_learning
    first 5 values: [-0.0417, -0.0558, 0.0347, -0.0, 0.017]
  id=8  score=0.2997  dim=384  topic=machine_learning
    first 5 values: [-0.0462, -0.0039, 0.0141, 0.0305, -0.0195]
```
...
=== Filtered: topic=machine_learning ===
  id=1  score=0.4123  topic=machine_learning  Machine learning algorithms learn patterns from data to mak...
  id=8  score=0.2867  topic=machine_learning  Gradient descent optimizes model parameters by iteratively ...

=== Filtered: topic=deep_learning ===
  id=6  score=0.1779  topic=deep_learning  Transformers use self-attention mechanisms to process seque...
  id=2  score=0.0689  topic=deep_learning  Neural networks are composed of layers of interconnected no...
```
...
| `points.search_batch(searches)` | Run multiple searches in one call |
...
| `vde.get_vector_count()` + `points.scroll()` | Count total and filtered points |
