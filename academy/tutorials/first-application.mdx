...
In this tutorial, you build a movie recommendation engine on Actian VectorAI DB from scratch. Along the way, you'll:

- Store movie plots as dense vectors and find semantically similar movies with natural-language queries
- Filter results by genre, year, or minimum rating
- Update movie metadata after ingestion and delete outdated records
- Inspect collection health and statistics
...
A user describes what they want to watch in natural language, for example "a suspenseful space movie", and the system finds the best matches from the database, optionally filtered by genre, year, or minimum rating. The diagram below shows how data flows from raw movie records through embedding and into a searchable vector store.
...
<Note>
On Windows, the default console codepage cannot render the `★` and `—` characters this script prints. If you see `UnicodeEncodeError` when running it, set `PYTHONIOENCODING=utf-8` before running, for example `set PYTHONIOENCODING=utf-8 && python app.py` (cmd) or `$env:PYTHONIOENCODING="utf-8"; python app.py` (PowerShell).
</Note>
...
Each step below builds on the ones before it: imports, the `model`, the `movies` dataset, and helper functions defined earlier are reused later. By the end you'll have a single runnable file (for example `app.py`). If you'd rather see the whole thing assembled first, jump to the [full example](#full-example) at the end of this page; every snippet below is excerpted directly from it.
...
| `m=16` | HNSW graph connections | Each node connects to 16 neighbours, balancing speed and recall. |
...
If no point has the given ID, `points.get()` returns an empty list rather than raising an exception, which is why the function checks `if not points` before indexing into the result. This matters for IDs that might not exist yet, for example right after a delete.
...
Payloads are schemaless, so `tags` didn't need to be declared anywhere in advance. Any JSON-serializable value works as a new field, which makes `set_payload` a convenient way to enrich points with metadata you didn't anticipate at ingestion time.
...
Use `vde.get_vector_count` for a fast unfiltered total, like the one above. Reach for `points.count()` (Step 14) when you need a count that matches a specific filter.
...
async def count_movies():
    async with AsyncVectorAIClient(url=SERVER) as client:
        ...
        # points.count() always performs a precise scan in the current SDK;
        # there is no exact/approximate toggle to pass here.
...
`points.count()` returns a plain integer: the precise number of points matching the optional `filter`. There is no `exact`/`approximate` toggle in this SDK version; every count is a precise scan.

### Expected output

Counts reflect the tutorial dataset after the rating update in Step 12 (Interstellar's rating became `8.8`) and the deletion of movie ID `9` in Step 13.
...
The code runs four separate counts: one for the full collection and three with filters applied. The first count returns the total number of points currently in the collection. The second filters by `genre == "sci-fi"`, the third by `rating >= 8.8`, and the fourth by `director == "Christopher Nolan"`. The `rating >= 8.8` count is `6` rather than `5` because Step 12 raised Interstellar's rating to `8.8`, adding it to the group alongside Inception, The Dark Knight, Pulp Fiction, Forrest Gump, and The Shawshank Redemption. If you run this step against a fresh collection without the Step 12 update applied, expect `5` instead.

---

## Step 15: Inspect collection status

The following snippet retrieves the collection's status, size, and current VDE lifecycle state, then prints them alongside the vector count. Run this at any point to verify the collection is healthy before running searches.
...
`get_info()` returns a `CollectionInfo` object. `status` and the VDE state are enums, so `.name.lower()` converts them to readable strings (for example `CollectionStatus.Green` becomes `"green"`). The table below describes the fields used above.

| Field | Meaning |
|-------|---------|
| `status` | Collection health: `green`, `yellow`, `red`, or `grey` (unknown). |
| `points_count` | Number of points currently stored. |
| `vectors_count` / `indexed_vectors_count` | Total vectors and how many are indexed for search. |
| `segments_count` | Number of storage segments backing the collection. |
| `optimizer_status.ok` | Whether the background optimizer is running without errors. |

### Expected output

A `green` status and `ready` VDE state indicate the collection is healthy and ready for searches.
...
Call `inspect_collection()` before automated jobs that assume the collection is ready. A status other than `green`, or a VDE state of `rebuilding`, means searches may return stale or incomplete results.
...
`collections.list()` takes no arguments and returns every collection on the server, not just ones your client created. That makes it useful for spotting orphaned collections from earlier runs before provisioning a new one.

---

## Step 17: Combine everything into a complete search function
...
Your scores, result ordering, and result counts will all vary. The `Filters` line always lists conditions in the order `genre`, `year`, `rating`, `NOT <exclude_genre>` (matching the order they're appended in the code above), and the `Results` count depends on how many points in your collection actually satisfy the filter, which can be smaller than `top_k` on a small dataset like this one. Three calls are made below with different queries and filter combinations.
...
```text
  ...
  Results: 5
    ...
    0.3180  The Matrix (1999) — sci-fi — ★8.7
           A computer hacker discovers that reality is a simulation created by machines and...
    0.2747  Alien (1979) — horror — ★8.5
           The crew of a commercial spaceship encounters a deadly extraterrestrial creature...
    0.2609  Inception (2010) — sci-fi — ★8.8
           A thief who steals corporate secrets through dream-sharing technology is given t...
    0.2284  Interstellar (2014) — sci-fi — ★8.8
           A team of explorers travel through a wormhole in space to ensure humanity's surv...
    0.2076  The Dark Knight (2008) — action — ★9.0
           Batman faces the Joker, a criminal mastermind who plunges Gotham City into anarc...
  ...
  Results: 4
    ...
    0.4317  Pulp Fiction (1994) — crime — ★8.9
           The lives of two mob hitmen, a boxer, a gangster, and his wife intertwine in fou...
    0.3621  The Shawshank Redemption (1994) — drama — ★9.3
           A banker sentenced to life in prison forms an unlikely friendship and finds hope...
    0.3185  Forrest Gump (1994) — drama — ★8.8
           A slow-witted but kind-hearted man from Alabama witnesses and unwittingly influe...
    0.3180  Inception (2010) — sci-fi — ★8.8
           A thief who steals corporate secrets through dream-sharing technology is given t...

  Query: "a feel-good movie about life"
  Filters: year>=1990, NOT crime
  Results: 3
    ...
    0.1866  Forrest Gump (1994) — drama — ★8.8
           A slow-witted but kind-hearted man from Alabama witnesses and unwittingly influe...
    0.1822  Interstellar (2014) — sci-fi — ★8.8
           A team of explorers travel through a wormhole in space to ensure humanity's surv...
```
...
Every snippet above is excerpted from the single script below; nothing new is introduced here. Save it as one file (for example `app.py`) and run it with `python app.py` for a single, runnable reference to the whole tutorial.

<Accordion title="See the complete script">
...
async def main():
    ...
    results = await search_movies("a suspenseful movie set in outer space")
    for r in results:
        p = r.payload
        print(f"  {r.score:.4f}  {p['title']} ({p['year']}) — {p['genre']} — ★{p['rating']}")

    genre_results = await search_by_genre("an exciting adventure", "sci-fi")
    for r in genre_results:
        p = r.payload
        print(f"  {r.score:.4f}  {p['title']} ({p['year']})")

    rated_results = await search_highly_rated("intense crime story", 8.8)
    for r in rated_results:
        p = r.payload
        print(f"  {r.score:.4f}  {p['title']} — ★{p['rating']}")

    advanced_results = await advanced_search("mind-bending thriller")
    for r in advanced_results:
        p = r.payload
        print(f"  {r.score:.4f}  {p['title']} ({p['year']}) — {p['genre']} — ★{p['rating']}")

    await get_movie(0)
    await update_movie_rating(0, 8.8)
    await get_movie(0)
    await add_tags(0, ["space", "wormhole", "survival", "time-dilation"])
    await get_movie(0)

    await delete_movie(9)
    # await delete_low_rated(8.6)  # destructive; uncomment only if you want to prune the dataset

    await count_movies()
    await inspect_collection()
    await list_collections()

    await recommend_movies("a mind-bending sci-fi movie")
    await recommend_movies("an intense crime story", min_rating=8.8)
    await recommend_movies("a feel-good movie about life", exclude_genre="crime", min_year=1990)
...
| Count | `points.count(collection, filter=...)` | Count matching points. |
| Vector count | `vde.get_vector_count(collection)` | Total vectors in the collection. |
| Collection info | `collections.get_info(collection)` | Status, point/vector counts, and segment info. |
